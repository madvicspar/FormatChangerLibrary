...
{
	public class RIS22FormattingStandardFactory : IFormattingStandardFactory
	{
	...
	{
		void SetNext(ICorrectionHandler next);
		Task Handle(WordprocessingDocument doc, FormattingTemplateModel template);
	}

	public interface IElementCorrectionStrategy
	{
		void ApplyCorrection(WordprocessingDocument doc, FormattingTemplateModel template);
	}

	public class CorrectionHandler : ICorrectionHandler
	{
		private ICorrectionHandler _next;
		private readonly IElementCorrectionStrategy _strategy;

		public CorrectionHandler(IElementCorrectionStrategy strategy)
		{
			_strategy = strategy;
		}

		public void SetNext(ICorrectionHandler next)
		{
			_next = next;
		}

		public async Task Handle(WordprocessingDocument doc, FormattingTemplateModel template)
		{
			_strategy.ApplyCorrection(doc, template);

			if (_next != null)
				await _next.Handle(doc, template);
		}
	}
}